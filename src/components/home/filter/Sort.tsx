...
function Filter() {
  ...
  return (
    <div tabIndex={0} role="button" className="w-full relative flex justify-end   p-2 lg:mr-20 items-center gap-2 text-slate-700">
      <div className="group w-3/12 flex items-center gap-2 p-3  ">
        <span className="text-base">Sort by:</span>
        <span className="font-bold text-base">{findLabel(searchParams.get('sort')!) || ''}</span>
        <SlArrowDown />
        <div className="absolute top-10 z-10 bg-white w-3/12   ">
          <ul className="hidden flex-col gap-2 p-3  shadow-lg left-0 group-hover:flex">
            {SORT_OPTIONS.map((option) => (
              <li key={option.id} className="hover:bg-slate-200 active:bg-slate-200 p-3 w-full cursor-pointer">
                <button onClick={() => handleSort(option)} type="button">{option.label}</button>
              </li>
            ))}
