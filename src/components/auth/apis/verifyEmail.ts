...
export const verifyEmail = async (token:string):Promise<any> => {
  ...
  try {
    const url = `${import.meta.env.VITE_BASE_URL}/api/v1/users/verify?token=${token}`;
    const response = await fetch(url, requestOptions);
    return await response.json();
  } catch (error:any) {
    throw new Error('We\'re unable to process your email verification request. Please try again later.');
  }
};
