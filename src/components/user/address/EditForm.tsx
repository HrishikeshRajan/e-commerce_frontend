...
import { updateAddressById } from '../../../utils/reduxSlice/appSlice';
    ...
    <>
      <Formik
        ...
        onSubmit={(values, actions) => {
          try {
            dispatch(updateAddressById({ address: values }));
            updateAddress({ ...values }, props.address._id).then((response) => {
              ...
              if (response.statusCode === StatusCodes.UNPROCESSABLE_ENTITY) {
                ...
                AuthHelper
                  .updateUserAddressInLocalStorage(values, props.address._id);
                AuthHelper.getUserFromLocalStorage();
              } else {
